...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--activation-site",
        choices=["block_output", "pre_mlp"],
        default="block_output",
        help=(
            "Capture decoder-block outputs, or post_attention_layernorm outputs "
            "immediately before each layer MLP."
        ),
    )
    parser.add_argument(
        "--roles",
...
def main() -> None:
    ...
    smoke = base.smoke_forward(
        model, tokenizer, prompts, layers, activation_site=args.activation_site
    )
    ...
    role_labels = [f"role_{role}" for role in roles]
    if args.activation_site == "block_output":
        ...
        labels = ["persona_assistant_axis"] + role_labels
        ...
        np.savez_compressed(
            args.output_dir / "centralized-paper-role-vectors.npz",
            selected_layer=np.asarray(selected_layer, dtype=np.int16),
            labels=np.asarray(role_labels),
            vectors=role_vectors,
            role_probe_directions=role_vectors,
        )
    selection = {
    ...
    base.write_json(
        args.output_dir / "run-metadata.json",
        {
            ...
            **selection,
            ...
            "assistant_axis": (
                str(args.axis) if args.activation_site == "block_output" else None
            ),
            "neutral_passages": str(args.passages),
            "activation_site": (
                "decoder layer output (residual stream after the full block)"
                if args.activation_site == "block_output"
                else "post_attention_layernorm output immediately before the layer MLP"
            ),
