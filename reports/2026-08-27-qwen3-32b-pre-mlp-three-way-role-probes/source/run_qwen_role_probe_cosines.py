...
def activation_module(model, layer_ix: int, activation_site: str):
    layer = model.model.layers[layer_ix]
    if activation_site == "block_output":
        return layer
    if activation_site == "pre_mlp":
        return layer.post_attention_layernorm
    raise ValueError(f"Unsupported activation site: {activation_site}")
...
def smoke_forward(
    ...
    for layer_ix in layers:
        handles.append(
            activation_module(model, layer_ix, activation_site).register_forward_hook(
                hook_for(layer_ix)
            )
        )
    try:
